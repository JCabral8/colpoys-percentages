...
- Full-show videos (v45): **▶ Friday show** / **▶ Saturday show** now play one rendered video each (`shows/firepit-hour-fri.mp4`, `shows/firepit-hour-sat.mp4`, 1080p, about 44 MB and 26 MB) instead of stepping through the clips with the live booth. `openReplay` mounts a `.vov` overlay on `document.body`, outside `#app`, so a re-render never restarts playback; the phone's player handles full screen and casting to a TV. The videos are built in the sibling `colpoys-show-hf` HyperFrames project (`tools/build_show.py fri|sat`, then render): booth with the app's jaw rig, karaoke captions, take card with pick flips on each player's name, count-up, W/L stamps, running season standings, bumpers. Re-render and replace the files if a clip or result changes. Same gate as before: a night appears only when none of its archive items is sealed (`replayOpen`).
